 ...
 * @link     http://pear.php.net/package/Numbers_Words
 */
class pl extends Words implements LocaleInterface
{
    /**
     * Locale name
     * @var string
     ...
     * @access public
     */
    private $def_currency = 'PLN'; // Polish zloty
     ...
     */
    function getWords($num, $power = 0, $powsuffix = '')
    {
        $ret = '';
            ...
            for ($p = $maxp; $p > 0; --$p) { // power
                ...
                if (isset($this->_exponent[$p])) {
                    ...
                    if ($snum !== '') {
                        ...
                        $ret .= $this->getWords($snum, $p, $cursuffix);
                    }
                    $curp = $p - 1;
                    continue;
                }
    ...
    function toCurrencyWords($int_curr, $decimal, $fraction = false, $convert_fraction = true)
    {
        ...
        $ret  = trim($this->getWords($decimal));
        $lev  = $this->getNumLevel($decimal);
        $ret .= $this->_sep . $curr_names[0][$lev];

        if ($fraction !== false) {
            if ($convert_fraction) {
                $ret .= $this->_sep . trim($this->getWords($fraction));
            } else {
     ...
     * @since  Numbers_Words 0.4
     */
    private function getNumLevel($num)
    {
        if (strlen($num) > 3) {
            $num = substr($num, -3);
        }
