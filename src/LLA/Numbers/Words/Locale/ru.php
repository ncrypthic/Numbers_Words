 ...
 * @link     http://pear.php.net/package/Numbers_Words
 */
class ru extends Words implements LocaleInterface
{
    /**
     * Locale name
     * @var string
     ...
     */
    public function getWords($num, $power = null, $powsuffix = null)
    {
        $dummy  = null;
        $gender = 1;
        ...
        return $this->getWordsWithCase($num, $dummy, $gender);
    }
     ...
     * @author Andrey Demenev <demenev@on-line.jar.ru>
     */
    private function getWordsWithCase($num, &$case, $gender = 1)
    {
        $ret  = '';
        $case = 3;
     ...
     * @author Andrey Demenev <demenev@on-line.jar.ru>
     */
    private function groupToWords($num, $gender, &$case)
    {
        $ret  = '';
        $case = 3;
    ...
    public function toCurrencyWords($int_curr, $decimal, $fraction = false, $convert_fraction = true)
    {
        ...
        $ret  = trim($this->getWordsWithCase($decimal, $case, $curr_names[0][0]));
        $ret .= $this->_sep . $curr_names[0][$case];

        if ($fraction !== false) {
            if ($convert_fraction) {
                $ret .= $this->_sep . trim($this->getWordsWithCase($fraction, $case, $curr_names[1][0]));
            } else {
