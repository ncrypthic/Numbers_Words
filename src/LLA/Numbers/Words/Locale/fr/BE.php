 ...
 */
namespace LLA\Numbers\Words\Locale\fr;
 ...
 * @link     http://pear.php.net/package/Numbers_Words
 */
class BE extends Words implements LocaleInterface
{
    ...
    function getWords($num = 0, $power = null, $powsuffix = null)
    {
        ...
        foreach ($num_groups as $i=>$number) {
            ...
            if ($number!='000') {
                if ($number!=1 || $pow!=2) {
                    $ret .= $this->showDigitsGroup($number, $i+1==$sizeof_numgroups||$pow>2).$this->_sep;
                }
