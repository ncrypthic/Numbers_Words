 ...
 */
namespace LLA\Numbers\Words\Locale\ro;

/**
 * Include needed files
 */
use LLA\Numbers\Words;
use LLA\Numbers\Words\LocaleInterface;
 ...
 * @link     http://pear.php.net/package/Words
 */
class RO extends Words implements LocaleInterface
{
    /**
     * Locale name.
     * @var string
     ...
     * @access public
     */
    public $lang_native = 'Română';


    /**
     * Specific noun configuration
     * @var array
     * @access private
     */
    public $noun;
    ...
    /**
     * Some currency names (as nouns, see {@link getWords()} in Romanian
     * @access private
     */
     ...
     * @access public
     */
    private $def_currency = 'RON'; // Romanian leu
     ...
     * @param array $noun        A noun, per {@link getWords()}
     ...
     * @author Bogdan Stăncescu <bogdan@moongate.ro>
     */
    function getNumberInflectionForGender($number_atom, $noun, $as_noun=false)
    {
     ...
     *
     * @param string $plural_rule The plural rule to use, per {@link getPluralRule()}
     * @param array $noun        A noun, per {@link getWords()}
     * @return string            The inflected form of the noun
     * @access private
     * @author Bogdan Stăncescu <bogdan@moongate.ro>
     */
    function getNounDeclensionForNumber($plural_rule, $noun)
    {
     ...
     * @param array $noun The noun representing the items being counted, per {@link getWords()}
     ...
     * @author Bogdan Stăncescu <bogdan@moongate.ro>
     */
    private function showDigitsGroup($num, $noun, $force_noun=false, $force_plural=false)
    {
        $ret = '';
        ...
        if ($s) {
            $ret.=$this->showDigitsGroup($s, $this->_exponent[2]);
            if ($uz) {
                $ret.=$this->_sep;
            }
        }
        if ($uz) {
            if (isset($this->_numbers[$uz])) {
              $ret.=$this->getNumberInflectionForGender($this->_numbers[$uz], $noun, !$force_noun);
            } else {
                if ($z) {
                ...
                if ($u) {
                    $ret.=$this->getNumberInflectionForGender($this->_numbers[$u], $noun, !$force_noun);
                }
            }
        }
        ...
        $plural_rule=$this->getPluralRule($num);
        if ($plural_rule=='o' && $force_plural) {
            $plural_rule='f';
        }

        return $ret.$this->_sep.$this->getNounDeclensionForNumber($plural_rule, $noun);
    }
     ...
     * @author Bogdan Stăncescu <bogdan@moongate.ro>
     */
    public function getWords($num = 0, $power = 0, $powsuffix = null)
    {
        if (empty($this->noun)) {
          $this->noun = array(NULL, NULL, Words::GENDER_ABSTRACT);
        }
        ...
        if (!$num || preg_match('/^-?0+$/', $num) || !preg_match('/^-?\d+$/', $num)) {
            $ret = $this->_numbers[0];
            if ($this->noun[2] != Words::GENDER_ABSTRACT) {
                $ret .= $this->_sep.$this->getNounDeclensionForNumber('f',$this->noun);
            }
            return $ret;
        }
        ...
        if (abs($num) == 1) {
            $ret = $this->getNumberInflectionForGender($this->_numbers[1], $this->noun);
            if ($this->noun[2] != Words::GENDER_ABSTRACT) {
                $ret .= $this->_sep.$this->getNounDeclensionForNumber('o',$this->noun);
            }
            return $ret;
        }
        ...
        foreach ($num_groups as $i=>$number) {
            ...
            if ($pow-1) {
               $ret.=$this->showDigitsGroup($number, $this->_exponent[($pow-1)*3]);
            } else {
               $showed_noun = true;
               $ret.=$this->showDigitsGroup($number, $this->noun, false, $num!=1);
            }
        }
        if (!$showed_noun) {
            $ret.=$this->_sep.$this->getNounDeclensionForNumber('m',$this->noun); // ALWAYS many
        }
    ...
    public function toCurrencyWords($int_curr, $decimal, $fraction = false, $convert_fraction = true)
    {
        ...
        $this->noun = $curr_nouns[0];
        $ret = $this->getWords($decimal);

        if ($fraction !== false) {
            $ret .= $this->_sep . $this->_and;
            if ($convert_fraction) {
                $this->noun = $curr_nouns[1];
                $ret .= $this->_sep . $this->getWords($fraction);
            } else {
                $ret .= $fraction . $this->_sep;
                $plural_rule = $this->getPluralRule($fraction);
                $this->getNounDeclensionForNumber($curr_nouns[1]);
            }
        }
