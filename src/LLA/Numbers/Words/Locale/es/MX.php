 ...
 */
namespace LLA\Numbers\Words\Locale\es;
 ...
 * @link     http://pear.php.net/package/Numbers_Words
 */
class MX extends Words implements LocaleInterface
{
    /**
     * Locale name
     * @var string
    ...
    public function toCurrencyWords($int_curr, $decimal, $fraction = false, $convert_fraction = true)
    {
        ...
        $ret = $this->_sep . ucfirst(trim($this->getWords($decimal) ." " . $ret)); 

        if ($fraction !== false) {
            if ($convert_fraction) {
                $ret .= $this->_sep .'con'. $this->_sep . trim($this->getWords($fraction));
            } else {
